...
**`make deploy` restarts ONLY the gateway — it is not the whole story (lm-54i).** Hermes is multi-process and loads the plugin into every plugin-hosting process at its own boot: the gateway, `hermes serve` (the desktop/Electron backend), and each `tui_gateway.slash_worker` (one per TUI session). `hermes plugins update` is just a **git pull** of the installed clone — it does **not** refresh any running process's memory, and Hermes has **no** cross-process plugin reload (there is `/reload-mcp` and `/reload-skills`, but no `/reload-plugins` — tracked upstream). So after a deploy, only the gateway runs the new build; the others keep the old one until they themselves restart.

For lifemodel this is now **mostly benign**: the brain loop and every turn hook (`pre_llm_call`/`post_llm_call`/`pre_gateway_dispatch`) run **only in the gateway** (turns funnel through it; `serve` is a control-plane that proxies to the gateway, `slash_worker` handles slash-commands only), and the platform `check_fn` reads no state (lm-54i) — so once every host runs this build or newer, a stale `serve` can no longer spam the `check_fn_state_read_failed` schema-skew WARNING. (First-rollout caveat: a `serve` still running a *pre-lm-54i* build keeps the old callback and must be restarted once to pick up the fix.) What can still run stale outside the gateway: a `/lifemodel` command in an **old TUI session** (restart that session), and — only in principle — turn hooks in a long-lived standalone `hermes` CLI that has lifemodel loaded and outlived the deploy (restart it). A change to command/hook behaviour that must land on the desktop backend or open TUI sessions needs those processes restarted too, not just `make deploy`.
