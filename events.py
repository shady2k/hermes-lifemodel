...
class EventRing:
    ...
    The freshness half of the trace pipeline (spec §4.2): where the durable
    :class:`~lifemodel.state.trace_store.TraceWriter` is the queryable source of
    truth, this in-process ``collections.deque`` is the ephemeral, not-yet-merged
    tail — read-your-writes for the viewer (before a flush lands) and ergonomic
    for test assertions. Synchronous, O(1) append, no I/O: the tick can project
    onto it without ever touching disk.

    A projection, never a source: :class:`~lifemodel.log.SpanLogger` appends here
    ONLY after the durable enqueue succeeds (durable-first), and each record
    carries the trace store's monotonic ``record_id`` so the viewer can overlay
    the ring on the flushed rows and dedup by it (spec §4.2, codex fix #5).

    This is NEW code sitting alongside the file-based :class:`EventSink` (Phase 1
    keeps both; ripping out ``events.jsonl`` is Phase 4). The ``emit``/``read``
    surface mirrors :class:`EventSink` so callers migrate without a shape change.
    """
    ...
    def emit(self, event: str, fields: Mapping[str, Any] | None = None) -> None:
        """Append a ``{"event": ..., **fields}`` record — :class:`EventSink` parity."""
        record: dict[str, Any] = {"event": event, **(dict(fields) if fields else {})}
        self.append(record)
