...
**Статус:** draft v0.5 · **Дата:** 2026-07-07
...
- **Что храним (обзор; детали — §4.1):** наше durable-состояние — это **одна память** за `MemoryPort` (домен-сущности фикс-каталога + операционное контакт-состояние: желания со статусом/отложенные, открытые петли, receptivity, ledger исходящих) плюс **витальный блоб** (быстрые скаляры `u`/энергия/усталость). **Идентичность/душа здесь НЕ хранится** — её ведёт Hermes (`SOUL.md`, §D2); наш genesis/becoming-слой — отдельная поздняя фаза.
...
Состояние плагина делится по **времени жизни и форме доступа** на **три дома**. Идентичность/душа сюда **не входит** — её ведёт Hermes (`SOUL.md`, §D2); наш genesis/becoming-слой — отдельная поздняя фаза.

| Дом | Что | Носитель | Живёт |
|---|---|---|---|
| **Сигналы** | эфемерные события сенсоров/давления | in-memory шина, TTL | секунды–минуты; не персистятся |
| **Сердцебиение** (витальное) | быстрые скаляры, переписываемые каждый тик: `u`/драйв, energy, fatigue, `last_tick` | атомарный JSON-блоб (`state.json`) | durable, без id/kind/lifecycle |
| **Память** (одна, за `MemoryPort`) | всё durable, что существо *знает/наработало*: сущности фикс-каталога + операционное контакт-состояние | несколько storage-классов за одним портом (churning → SQLite stdlib) | durable, с lifecycle/decay |

Витальное физически вне памяти, но концептуально — **часть само-модели**: когниция получает его в брифе как «текущее телесное состояние».
...
- **Drive (`u`)** — скалярное накопительное давление; витальное, **не** сущность; владелец — 0-LLM нейрон.
- **Desire** — рождается агрегацией при переходе порога; **два субдокумента одного объекта** (против two-writer):
  - **`DesireActivation`** (0-LLM, агрегация): `drive_source`, `strength`, `status{active|deferred}`, `threshold_crossed_at`;
  - **`DesireFrame`** (LLM, когниция): `about`, `because`, `candidate_reasons`, `normative_read`;
  - общие поля — версия/CAS.
...
**Операционное контакт-состояние** (durable, живёт в памяти рядом; не «сущность-с-содержанием»): дайджест последнего обмена (ts/тон/receptivity), ≤3–5 открытых петель, ledger исходящих (`unanswered_outbound_count`, `time_since_last_outbound`, `last_outbound_topic`, `has_new_reason_since`, `quiet_mode_until`), backoff. Без него скользящая лестница контакта (§11) **не социальна**.

**Владение и поток:**
1. Нейрон (0-LLM) → сигнал.
2. Агрегация (0-LLM): сигналы → salience/давление → будить? Владеет `DesireActivation`; содержание не пишет. Читает **не память целиком, а компактный `pressure-index`** — детерминированную проекцию (силы активных желаний, счётчик pending-intention, топ-salience петель, статус неотвеченного, ближайший eligible-wake, макс контакт-давление). Thought'ы каждый тик **не** сканирует.
3. Когниция (LLM, по пробуждению): минтит/обновляет `DesireFrame` (позже — Thought/Intention); формирует Intention-решение; исполняет / откладывает / молчит.
...
**hindsight** — внешний семантический recall *поверх* нашего содержания (поисковый слой), **не** отдельный source-of-truth.

**Отложено (осознанно, до фундамента внутренней жизни):** полный Thought-субстрат + SQLite entity-store; durable Intention-lifecycle; семантика не-ответа (молчание ≠ отказ/приглашение); полный outbound-ledger с валентностью ответа; обучение предпочтениям (каденс/часы/темы); privacy-классификатор чувствительности мыслей; аудит «почему написал»; decay/забывание.
...
- **D7. Объектная модель и хранение — РЕШЕНО (v0.5, §4.1).** Три дома: сигналы (эфемерная шина) · сердцебиение/витальное (быстрый атомарный блоб `u`/энергия/усталость) · одна память за `MemoryPort` (несколько storage-классов; churning-сущности → **SQLite** stdlib). Каталог `kind` **закрыт** (агрегация типы не выдумывает). Цепочка **Drive → Desire(`Activation` 0-LLM / `Frame` LLM) → Intention**(пока концепт-решение, персист позже) **+ Thought**(Ф6). Агрегация читает **`pressure-index`**, не стор целиком. Идентичность/**душа — у Hermes** (`SOUL.md`, D2) + genesis/becoming позже; здесь не хранится. **hindsight** — recall-слой, не source-of-truth. Родословная (оригинальный lifemodel) подтверждает паттерн: агрегация держит только эфемерное, домен-сущности — в отдельном сторе, читаются как давление.
...
_Draft v0.5 (2026-07-07) — + §4.1 объектная модель и три дома состояния (Drive→Desire[`Activation`/`Frame`]→Intention+Thought · фикс-каталог `kind` · `pressure-index` · `MemoryPort`/storage-классы) и D7; душа делегирована Hermes до фазы души. Ревью Codex ×4. Готово к writing-plans._
