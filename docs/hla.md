...
**Статус:** draft v0.7 · **Дата:** 2026-07-07
...
### 4.1 Объектная модель и три дома состояния

Состояние делится по **времени жизни и форме доступа** на **три логических дома**; физически durable-часть — **один SQLite** (`lifemodel.sqlite`), эфемерная — шина в памяти. Идентичность/душа сюда **не входит** — её ведёт Hermes (`SOUL.md`, §D2); наш genesis/becoming-слой — отдельная поздняя фаза.
...
| **Память** (сущности, за `MemoryPort`) | durable-**сущности** фикс-каталога с `id`/`kind`/жизненным циклом/запросом: Desire-запись (позже Intention/Thought) | таблица `memory_records` в `lifemodel.sqlite`, за `MemoryPort` | durable, с lifecycle/decay |
...
**Каталог сущностей — фиксированный** (агрегация типов в рантайме не выдумывает — подтверждено родословной: закрытый набор + закрытые enum'ы сигналов). Ядро цепочки — **Drive → Desire → Intention**, плюс **Thought**:

- **Drive (`u`)** — скалярное накопительное давление; витальное (строка `runtime_state`), **не** сущность; владелец — 0-LLM нейрон. Робастность рефлекса теперь = надёжность SQLite (WAL) + recovery на старте (§ниже), а **не** отдельный fallback-файл.
- **Desire** — рождается агрегацией при переходе порога; **ОДНА запись** `memory_records` (не расщепляется). Один стор ⇒ активацию, содержание и витальное можно писать **в одной транзакции**. Два *аспекта* одной записи, два владельца-писателя:
  - **`DesireActivation`** (0-LLM, агрегация) — колонки жизненного цикла: `state{active|deferred|resolved|archived}`, `strength`, `drive_source`, `threshold_crossed_at`;
  - **`DesireFrame`** (LLM, когниция) — `payload`: `about`, `because`, `candidate_reasons`, `normative_read` — дописывается через `transition(... to_state='active', patch=frame)`.
  - **`desire_status` — НЕ поле `runtime_state`** — это `state` записи-Desire; агрегация видит его только через `pressure-index` (§ниже).
- **Intention** — обязательство (Bratman) из желания. **Пока концепт, не персистентный объект:** эфемерное решение `ACT_SEND | ACT_SILENT | ACT_DEFER_UNTIL`. Персистим при нужде в durable-deferral / reconsideration / post-eval.
- **Thought** — содержательный субстрат (память/вопрос/благодарность/кандидат-фраза); копится в простое (Zeigarnik); даёт бессодержательному желанию *что* сказать и переживает host-сброс сессии. **Поздняя фаза (Ф6).**
- *(позже)* Commitment · Opinion · Prediction — те же правила.

**Операционное контакт-состояние — по правилу границы, НЕ в памяти сущностей:** безымянные скаляры/таймеры контроля (`unanswered_outbound_count`, `time_since_last_outbound`, `last_outbound_topic`, `has_new_reason_since`, `quiet_mode_until`, backoff, `last_exchange_at`) живут в строке **`runtime_state`** (за `StatePort`) рядом с витальным — часто переписываются, без identity/lifecycle; в `memory_records` их **не** дублируем (богатый ledger ответов — `outbound_ledger`, отдельно). Что реально сущность с жизненным циклом — **≤3–5 открытых петель** и богатый дайджест обмена — это `memory_records`, и это **Ф6**. Без операционного слоя скользящая лестница контакта (§11) **не социальна**.

**Владение и поток:**
1. Нейрон (0-LLM) → сигнал.
2. Агрегация (0-LLM): сигналы → salience/давление → будить? Владеет жизненным циклом `DesireActivation`; содержание не пишет. Память **не читает напрямую** — только компактный `pressure-index` через **отдельный `PressureSensorPort`** (SRP: агрегация физически не может `put`/`find` контент). `pressure-index` считается **на момент запроса** — параметризованный SELECT с `now` от `ClockPort` (не статичный SQL-VIEW: тестируемость на фейк-часах + tz-корректность через epoch-колонки), уважает `expires_at > now` (не протухает, пока когниция спит). WAL → чтение не блокирует писателя; БД битая/недоступна → recovery на старте (§ниже) + **fail-soft** на тике (`contact_frame_available=False`), **не** restart-loop. Thought'ы каждый тик **не** сканирует.
3. Когниция (LLM, по пробуждению): минтит/обновляет `DesireFrame` через `MemoryPort.transition` (позже — Thought/Intention); формирует Intention-решение; исполняет / откладывает / молчит.
4. Состояние → `pressure-index` → следующий цикл.
...
- **`MemoryPort`** — CRUD сущностей для когниции: `put(record) -> id` · `get(kind, id)` · `find(kind=None, state=None, limit=None, order_by=None)` · `transition(kind, id, from_state, to_state, patch=None)`. **Удаление — мягкое** (`transition(... to_state='archived')`); hard-`purge` — с «забыванием» (Ф6). Семантический поиск — **опциональная capability**.
...
**Отложено (осознанно, до фундамента внутренней жизни — Ф6):** *сам* SQLite-стор + Desire-запись строим сейчас (узкий аванс), но откладываем — Thought-субстрат и thought-gen; семантический/векторный поиск, граф, `fts5`; сон-консолидация/decay/забывание (hard-`purge`); durable Intention-lifecycle; семантика не-ответа (молчание ≠ отказ/приглашение); полный outbound-ledger с валентностью ответа; обучение предпочтениям (каденс/часы/темы); privacy-классификатор чувствительности мыслей; аудит «почему написал».
...
**Проактивный промпт когниции оборачивает wake-packet** (качество формулировки — решающее, bead `lm-pbm`):
