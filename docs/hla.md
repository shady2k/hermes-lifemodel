...
| **Signal bus** | **эфемерный `SignalFrame` (in-memory) внутри `ExecutionFrame`** — НЕ durable-лог; сенсоры — продюсеры, агрегация — консюмер. Пустой после рестарта. Идемпотентность внешних событий — не курсором шины, а ring `processed_external_event_ids` в `AgentState` (см. core-rethink 2026-07-11) | нет |
...
| **Cognition** | пробуждённое мышление: быстрая модель → умная по confidence; **act-gate = собственный ход существа**, исход хода — `proactive_outcome` (`sent`/`silent`/`failed`/`stale`); действует в мире ТОЛЬКО этот слой | да |
...
1. **Рождение.** Первый переход порога → агрегация создаёт **одно желание контакта** и будит когницию. Значение нейрона — **насыщающее давление** (растёт в тишине, сбрасывается только реальным контактом); длительность депривации трекается временем отдельно. *(⚠ Пересмотрено D10/rebuild: нейрон — мгновенный сенсор; насыщающее `u` копит отдельный AUTONOMIC drive-интегратор, не нейрон.)*
...
> **Резолв §2↔§4 (core-rethink 2026-07-11).** §2-таблица раньше называла шину «durable append-only лог» — это была **ошибка**; авторитетна модель §4: **сигналы эфемерны, in-memory, не персистятся.** Уточнения пересборки: (1) единица обработки — **`ExecutionFrame`** (heartbeat ∨ входящее событие ∨ завершение async-когниции ∨ admin), сигнал живёт **≤ один frame**, коммит — в конце frame, все frame-ы сериализованы одним state-actor; (2) durable-домены: `AgentState` («я», скаляры) + `Memory`/BDI + `UserModel` («другой», вывод/кэш с `inferred_at`+TTL) в `lifemodel.sqlite`; `Trace` — отдельная `observability.sqlite`; сырьё истории — провайдер (hindsight/Honcho), не наш дубль; (3) **`Relationship` → `UserModel`** (переименование, миграция последней); идемпотентность внешних событий — ring `processed_external_event_ids` в `AgentState`, не курсор шины. Полный дизайн: [`superpowers/specs/2026-07-11-sensory-core-rethink-design.md`](superpowers/specs/2026-07-11-sensory-core-rethink-design.md).
...
**Relationship/Receptivity — субстрат уместности** (needed-now: «не назойливо» = архитектура, не вайб; grounds: perceived responsiveness · relationship-maintenance · politeness/face · communication-privacy-management · expectancy-violations). Поля (**выученные нормы/паттерны + derived receptivity**, не оперативные счётчики): каденс, хорошие/плохие часы, паттерн валентности откликов, границы privacy, чувствительность тем, глубина близости, норма задержки ответа, известная нагрузка, допустимые стили, явные предпочтения. Населяется **и спрашивая, и выводя** (как в жизни). **Appraisal-гейт** (relevance/congruence/coping/norm-fit — Lazarus/Scherer) читает её как **вычисление, не объект**; `Expectation` (вероятный отклик/уверенность) — **поля**, не объект. **Живые egress-счётчики** (backoff, `unanswered_outbound_count`, `action_pending`, **`silence_anchor_at`** — настраиваемый якорь active-silence-окна: `force_wake` бэкдейтит его за окно, `satiate` ставит `now`, `set` пишет; `None` ⇒ фолбэк на реальный `last_exchange_at`, подлинный обмен его **очищает**) остаются в `runtime_state` — Relationship их **читает**, но **не** держит второй durable-ledger (иначе split-brain). **`last_exchange_at` — не настраиваемая ручка, а иммунная запись реального двустороннего обмена** (lm-md6.1): пишется **только** подлинным обменом (`core/aggregation.py`), рендерит темпоральный факт wake-пакета (`render_temporal_facts`) и **иммунна к admin/control-командам** — диадическое **свидетельство**, не счётчик.

**Три фасета само-модели (решение lm-md6.2) — концептуальная ось, ОРТОГОНАЛЬНАЯ разбиению на порты хранения** (`StatePort`/`MemoryPort`/`PressureSensorPort` — ось **ХРАНЕНИЯ**; фасеты — ось **СМЫСЛА**, они не совпадают). Само-модель существа именует **три** дома вопросов:

| Фасет | Вопрос | Что | Хранение |
|---|---|---|---|
| **Автономный-Я** | «как я?» | `u`/`duration_over_theta`/energy/fatigue/`last_tick` | `StatePort` (`runtime_state`) |
| **Отношение/Другой** | «как они / как мы?» | **выученная** receptivity (нормы/паттерны + derived), не оперативные счётчики | BDI-объект, `MemoryPort` |
| **Воплощение/Субстрат** (Hermes) | «могу ли дотянуться / дошло ли / что умею?» | операционные факты тела/канала: `pending_proactive_*`, `action_pending`, `proactive_send_log`, `unanswered_outbound_count`, `last_contact_at`, `silence_anchor_at`, delivery-confirmation, reachability/доступность адаптера | `StatePort` (`runtime_state`) |

Значит `StatePort`/`runtime_state` физически держит **ДВА** фасета (автономный-Я + воплощение); третий (отношение) — объект `MemoryPort`. **`last_exchange_at` — граничный объект (boundary object):** диадическое **свидетельство** на границе канал+Другой — авторитетно в `runtime_state` (агрегация читает каждый тик), пишется **только** реальным обменом, Relationship **читает-но-не-владеет** (не эксклюзивно ни одному фасету: телу принадлежит канал, Другому — сам факт).

**Инвариант (анти-паттерн «эпистемическая конфляция»):** **НЕ** схлопывать `verdict` / `delivery` / `reply` / `exchange` в один концепт «контакт случился» — это **класс** багов (lm-md6.1: `force_wake` подделал запись обмена, и модель услышала «last exchange 20 минут назад»; lm-2gi: счёт по `verdict` LLM вместо **подтверждённой** доставки).

**Физическое ограничение (без изменений, D7/§4.1):** расщепление — **концептуальное/именование**, не физическое: один блоб `runtime_state`, один `StatePort`, **никакого** нового порта/таблицы/леджера (второй durable-ledger = split-brain, запрещён). Отдельный **`EmbodimentPort`** оправдан **только** если факты воплощения когда-нибудь обзаведутся identity/lifecycle/query-давлением (то самое правило границы домов).
...
- **D2. Инъекция soul-слоя — ~~РЕШЕНО~~ ОТМЕНЕНО (ADR-0002, 2026-07-13).** ~~Решение: (1) входящий ход — `pre_llm_call`-хук инжектит текущий soul; (2) проактивный ход — soul в *нашем* промпте; (3) базовый `SOUL.md` — тонкая «уступающая» персона, в рантайме не правится.~~ **Отменено:** факт, на котором D2 стоял, верен (системного хука на промпт нет — `VALID_HOOKS`, `hermes_cli/plugins.py:134`; `pre_llm_call` вклеивает в *user-сообщение*, `agent/turn_context.py:431`), но **вывод из него был неверен**. Инжектить душу в user-сообщение значит каждый ход присылать существу, кем оно является, **довеском к реплике собеседника**: структурно это «контекст», а не «кто ты есть». **Душа = `SOUL.md`** (слот #1 системного промпта, `load_soul_md`, `agent/prompt_builder.py:1819`, читается заново каждый ход), и ведём её мы — read-modify-write, файл всегда сам себе база, никогда не стираем. «Уступающая персона» не заводится. `pre_llm_call` остаётся за нами для **эфемерного** (ambient felt-cue, блок `<genesis>`) — манера и ситуация, не идентичность. Детали и риски (сканер угроз, эрозия, атомарность) — ADR-0002.
- **D3. Формат и расположение — РЕШЕНО (уточнено D7/v0.7).** Наш durable runtime — **один SQLite** `lifemodel.sqlite` (не JSON-блоб). Человекочитаемый Markdown — это **`SOUL.md`**: человеко-редактируемая prose идентичности/характера, которую **ведём мы** (ADR-0002 — отменяет D2). Прозу идентичности `lifemodel.sqlite` **не хранит** (только ревизии души + бухгалтерию genesis). Живёт под **домом профиля** (cron анкерится на `get_hermes_home()` = профиль) → бэкап профиля забирает всё.
...
- **D9. Единая спина рождения сущностей (single-spine) — РЕШЕНО (v0.9; codex ×3 + внешний архитектор ×3 + родословная оригинального `lifemodel`).** **⚠ Рамка ПЕРЕСМОТРЕНА → см. D10 (rebuild, v1.0): «инкрементально / parity перед заменой / `sim/` не трогаем» отменено; «нейрон копит давление» заменено.** Полная дизайн-запись: [`docs/superpowers/specs/2026-07-08-core-single-spine-design.md`](superpowers/specs/2026-07-08-core-single-spine-design.md). **Проблема:** сейчас два независимых механизма рождения (`ContactAggregation` из нейрона; и **отдельный** `ThoughtGeneration` со своими триггерами event/idle/chaining + своим рейт-лимитингом) → шумовые/дублирующиеся мысли, спираль тишины, два гейтинга = не отладить (наблюдено вживую 2026-07-08). **Решение:** одна каузальная спина `сенсор(нейрон) → сигнал → generic Aggregation-раннер → cognition/act → intent-шина → SQLite`; ничто не рождает durable-сущности вне неё; `ThoughtGeneration` растворяется в спину. Механику событий/порогов портируем из оригинального `lifemodel`. **Строим ИНКРЕМЕНТАЛЬНО** — механизм ядра добавляем, когда фича его требует (YAGNI: сегодня 1 драйв контакта + тонкие event-мысли; generic-реестр/ack/TTL/агрегация/Вебер — по нужде). **Инварианты (обязательны):** (1) **контакт — абсолютно-сертифицированный** wake (`sim/wake`), никакой relative/Вебер-фильтр не подавляет `u ≥ θ`; parity-тесты перед заменой; (2) нейроны — плоские измерения, взвешивание+рождение — в политике; (3) семантический ack — через SQLite intent/committer, **не** `signals.consumed`; (4) если TTL: `TTL > max(cooldown, deferral)`, control-сигналы не прунятся; (5) **хранение мыслей ≠ проекция промпта** (проекция фильтрует silence-biased/низко-релевантные; скрытая мысль **не влияет на `u`**); (6) дедуп — структурный (сигнал раз/`origin_id` + не рождать живой singleton) + **live content-fingerprint** для мыслей (не глобальный хеш — иначе амнезия); (7) **chaining/idle-руминация — из быстрого тика в Фазу 6**; (8) `sim/` не трогаем. **Отклонено:** полный порт control-theory сразу (YAGNI), булеан-флаг `prompt-visible` (→ проекция), глобальный контент-хеш (→ live-fingerprint), `RuminationNeuron` на «только тишине» (подмножество контакт-сенсора). Обобщает §2.1 (жизненный цикл желания) на все сущности.
- **D10. Пересборка ядра (single-spine rebuild) — РЕШЕНО (v1.0; codex ×2 с доступом к репо; переопределяет рамку D9).** Полная запись: [`docs/superpowers/specs/2026-07-08-core-single-spine-design.md`](superpowers/specs/2026-07-08-core-single-spine-design.md); высокоуровневый обзор — артефакт `core-rebuild-overview`. **Причина:** живая сущность **ни разу** не написала владельцу (каждый проактивный ход → `[SILENT]`); диагностика вскрыла три слоя: `sim/` сертифицирует **не тот код** (`sim/harness.py` — третья реализация тика, а не живой конвейер), сам wake-packet-**промпт учит молчать** («если наполнитель — `[SILENT]`»), а ветка «тоска-как-повод» недостижима по калибровке (0.45 < 0.75). **Рамка D9 (инкрементально раствори + parity перед заменой) ОТМЕНЕНА:** parity замораживает «всегда молчит» как эталон → бетонирует баг. **Решение — пересборка поведенческого слоя** до минимума, который функционирует и полностью наблюдаем, при **сохранении** storage/спины-фундамента (D7/D8). **Инварианты (заменяют/дополняют D9):** (1) сенсоры — **строго мгновенные, без durable-state**; накопление/владение `u` — отдельный AUTONOMIC drive-интегратор; политика рождения — в агрегации *(пересматривает §2.1/§4)*; (2) **симуляция гоняет реальный код** через fake-порты — единственная правда — реальные компоненты, параллельной модели тика нет *(разворачивает D9-инвариант «`sim/` не трогаем»: `sim/harness.py`+`sim/aggregation.py` удаляем, `Drive`/`evaluate_wake` вживляем, старый sim → unit-регрессии)*; (3) **наблюдаемость принудительна** — лог без trace+span невозможен, каждое решение (вкл. не-действие) — span с `reason` из закрытого enum; (4) act-gate — **асинхронный ход Hermes**, не синхронный LLM в core (`CognitionLauncher` лишь резервирует энергию + строит wake-packet + `LaunchProactive`); приёмка по реальному `proactive_outcome`, не `delivered`; (5) `[SILENT]` лечится **промпт-контрактом + достижимой калибровкой + залогированным молчанием**; (6) безопасность/ненавязчивость (backstop / in_flight / pending-idempotency / анти-повтор) — **сохраняем**, барочность (Рубикон / receptivity-appraisal / jitter / thought idle+chaining) — режем под honest suppression-span. **Storage/объектная модель (D7/D8) остаётся**; откладывается лишь богатая **поведенческая** машинерия (Рубикон/receptivity/деревья мыслей → по нужде). Контакт — первый щуп; приёмка = живое проактивное сообщение владельцу.
