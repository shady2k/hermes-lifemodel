...
def decide(state, turn, params, now):               # только light cue
    if not warmed(state, now):                       # cold-start (lm-z2e) — жёстко нет
        return NONE
    if (is_salient(v, a, params)                     # достаточно ярко (не просто не-нейтрал)
            and not is_task_context(turn)            # только надёжные поведенческие сигналы
            and (felt_changed(state) or cooldown_elapsed(state, now, params))):
        return LIGHT
    return NONE
```
...
- **is_task_context(turn)** — **только надёжные, язык-независимые** сигналы (моя правка против
  NL-ключевиков codex): недавний ассистент-ход с `tool_calls`; в сообщении/недавних ходах —
  код-фенсы, стек-трейсы, диффы (`^@@`, `^\+\+\+`), пути к файлам, JSON/YAML, shell-команды,
  логи, длинная вставка (> порог символов). Окно последних K сообщений (естественный декей —
  заменяет «task-streak» codex'а без риска застрять навсегда).
